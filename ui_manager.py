...
class UIManager:
    ...
    def create_interface(self):
        ...
        self.container.pack(fill='both', expand=True,
                            padx=UIConstants.PADDING_MAIN, pady=UIConstants.PADDING_MAIN)
        ...
        main_container.pack(fill='both', expand=True,
                            pady=(UIConstants.PADDING_MAIN, 0))
        ...
        if self.messages_scroll:
            ...
            self.message_manager.add_message(
                "Sistema iniciado e pronto para uso", 'info')

    def _create_header(self):
        """Cria header moderno"""
        header = ctk.CTkFrame(
            self.container, fg_color="transparent", height=140)
        header.pack(fill='x')
    ...
    def _create_automation_list(self, parent):
        ...
        title_list.pack(pady=(UIConstants.PADDING_MAIN, 10),
                        padx=UIConstants.PADDING_MAIN, anchor='w')
        ...
        self.scroll_frame.pack(fill='both', expand=True, padx=UIConstants.PADDING_MAIN, pady=(
            0, UIConstants.PADDING_MAIN))

    def _create_details_panel(self, parent):
        """Cria painel de detalhes"""
        details_frame = ctk.CTkFrame(
            parent, **UIConfig.get_frame_style("card"))
        details_frame.grid(row=0, column=1, sticky="nsew")

        # Seção de detalhes (altura fixa para consistência)
        details_section = ctk.CTkFrame(
            details_frame, **UIConfig.get_frame_style("section"))
        details_section.pack(fill='x', padx=UIConstants.PADDING_MAIN, pady=(
            UIConstants.PADDING_MAIN, 10))
        # Impede redimensionamento automático
        details_section.pack_propagate(False)
        details_section.configure(
            height=UIConstants.DETAILS_SECTION_HEIGHT)  # Altura fixa
        ...
        title_details.pack(pady=(UIConstants.PADDING_SECTION, 10),
                           padx=UIConstants.PADDING_MAIN, anchor='w')
        ...
        self.automation_selected_label.pack(
            pady=(0, 8), padx=UIConstants.PADDING_MAIN)
        ...
        self.last_execution_label.pack(
            pady=(0, UIConstants.PADDING_MAIN), padx=UIConstants.PADDING_MAIN)
        ...
        messages_section = ctk.CTkFrame(
            details_frame, **UIConfig.get_frame_style("section"))
        messages_section.pack(fill='both', expand=True, padx=UIConstants.PADDING_MAIN, pady=(
            10, UIConstants.PADDING_MAIN))
        ...
        title_messages.pack(pady=(UIConstants.PADDING_SECTION, 10),
                            padx=UIConstants.PADDING_MAIN, anchor='w')
        ...
        self.messages_scroll.pack(fill='both', expand=True, padx=UIConstants.PADDING_MAIN, pady=(
            0, UIConstants.PADDING_MAIN))
    ...
    def create_automation_card(self, name: str, description: str = "", status: str = "Pronto") -> AutomationCard:
        ...
        content_frame.pack(fill='both', expand=True,
                           padx=UIConstants.PADDING_CARD, pady=12)
        ...
        self._setup_card_events(
            card, name_label, card.status_label, content_frame, main_container, info_container)
        ...
        def on_enter(event):
            if self.selected_automation != card.name:
                card.card_frame.configure(
                    fg_color="gray95", border_color="gray60")

        def on_leave(event):
            if self.selected_automation != card.name:
                card.card_frame.configure(
                    fg_color="white", border_color="gray70")
    ...
    def set_callbacks(self, callbacks: Dict[str, Callable]):
        ...
        if self.btn_execute_main and 'execute_automation' in callbacks:
            self.btn_execute_main.configure(
                command=callbacks['execute_automation'])
    ...
    def select_automation(self, name: str):
        ...
        if self.selected_automation:
            for card in self.automation_cards:
                if card.name == self.selected_automation:
                    card.card_frame.configure(
                        fg_color="white", border_color="gray70", border_width=1)
                    card.indicator.configure(
                        fg_color=UIColors.get_status_color(card.status_original))
                    break
        ...
        for card in self.automation_cards:
            if card.name == name:
                card.card_frame.configure(
                    fg_color="lightblue", border_color=UIColors.ACCENT, border_width=2)
        ...
        if self.automation_selected_label:
            self.automation_selected_label.configure(
                text=name, text_color="gray10")
        ...
        for card in self.automation_cards:
            if card.name == name:
                ...
                if status == 'Executando':
                    card.card_frame.configure(
                        border_color=UIColors.EXECUTING, border_width=2)
                elif self.selected_automation == name:
                    card.card_frame.configure(
                        border_color=UIColors.ACCENT, border_width=2)
                else:
                    card.card_frame.configure(
                        border_color="gray70", border_width=1)
                break

    def update_automation_counter(self, count: int):
        """Atualiza contador de automações"""
        if self.automation_counter:
            self.automation_counter.configure(
                text=f"{count} automações disponíveis")
    ...
    def update_last_execution(self, info: str):
        ...
        if self.last_execution_label:
            self.last_execution_label.configure(
                text=f"Última execução: {info}")
    ...
    def _animate_token_spinner(self) -> None:
        ...
        current_frame = spinner_frames[self.token_spinner_frame % len(
            spinner_frames)]
