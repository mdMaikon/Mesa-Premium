...
class ExecutionManager:
    ...
    def execute_automation(self, automation_name: str) -> None:
        ...
        if automation_name in self.active_processes:
            self.message_manager.add_message(
                f"Automação '{automation_name}' já está em execução", 'warning'
            )
            return
        ...
        if not self.automacao_manager.obter_automacao(automation_name):
            self.message_manager.add_message(
                f"Automação não encontrada: {automation_name}", 'error'
            )
            return
    ...
    def execute_token_renewal(self) -> None:
        ...
        self.message_manager.add_message("🔑 Iniciando renovação de token...", 'info')
    ...
    def _execute_automation_thread(self, automation_name: str) -> None:
        ...
        try:
            self.message_manager.add_log_entry(f"Iniciando execução modular: {automation_name}")
            self.message_manager.add_message(f"Iniciando {automation_name}...", 'info')
    ...
    def _process_automation_result(self, automation_name: str, resultado: Any) -> None:
        ...
        if resultado and isinstance(resultado, dict) and resultado.get('success', False):
            ...
            self.message_manager.add_message(f"✅ {mensagem_sucesso}", 'success')
            if self.status_update_callback:
            ...
            self.message_manager.add_message(f"❌ {mensagem_erro}", 'error')
            if self.status_update_callback:
                self.status_update_callback(automation_name, "Erro", 'error')
        else:
            # Para automações que não retornam estrutura de sucesso/falha padrão
            self.message_manager.add_message(f"✅ {automation_name} executado", 'success')
            if self.status_update_callback:
                self.status_update_callback(automation_name, "Concluído", 'success')
    ...
    def _handle_automation_error(self, automation_name: str, error: Exception) -> None:
        ...
        self.message_manager.add_log_entry(f"Erro durante execução modular de {automation_name}: {str(error)}")
        self.message_manager.add_message(f"❌ Erro inesperado em {automation_name}", 'error')
    ...
    def _handle_token_error(self, error: Exception, execution_id: Optional[int]) -> None:
        ...
        self.message_manager.add_log_entry(f"Erro durante execução de token: {str(error)}")
        self.message_manager.add_message(f"❌ Erro inesperado na renovação de token: {str(error)}", 'error')
    ...
    def get_last_execution(self, automation_name: str) -> str:
        ...
        try:
            ...
            self.message_manager.add_log_entry(f"Erro ao obter última execução: {str(e)}")
