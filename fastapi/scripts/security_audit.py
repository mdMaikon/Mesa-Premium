...
class SecurityAuditor:
    ...
    def run_pip_audit(self) -> dict[str, Any]:
        ...
        try:
            ...
            if result.returncode not in [
                0,
                1,
            ]:  # 1 is expected when vulns found
            ...
            for severity, keywords in severity_keywords.items():
                if any(keyword in description for keyword in keywords):
                    if severity == "critical":
                        ...
                        highest = "high"
                    elif severity == "medium" and highest not in [
                        "critical",
                        "high",
                    ]:
                        highest = "medium"
    ...
    def generate_report(
        self, audit_data: dict[str, Any], analysis: dict[str, Any]
    ) -> str:
    ...
    def _generate_text_report(
        ...
        report.append(
            f"Vulnerable packages: {analysis['vulnerable_packages']}"
        )
        report.append(
        ...
        report.append("-" * 20)
        report.append(
            f"🔴 Critical: {analysis['by_severity']['critical']} packages"
        )
        report.append(f"🟠 High: {analysis['by_severity']['high']} packages")
        report.append(
            f"🟡 Medium: {analysis['by_severity']['medium']} packages"
        )
        ...
        if analysis["critical_packages"]:
            ...
            for pkg in analysis["critical_packages"]:
                report.append(
                    f"• {pkg['name']} {pkg['version']} ({pkg['vulns']} vulnerabilities)"
                )
            report.append("")
            ...
            for package in audit_data["dependencies"]:
                ...
                if vulns:
                    ...
                    for vuln in vulns:
                        ...
                        if vuln.get("aliases"):
                            report.append(
                                f"    Aliases: {', '.join(vuln['aliases'])}"
                            )
                        if vuln.get("fix_versions"):
                            report.append(
                                f"    Fix available: {', '.join(vuln['fix_versions'])}"
                            )
        ...
        if analysis["recommended_updates"]:
            ...
            for pkg, update in analysis["recommended_updates"].items():
                report.append(
                    f"• {pkg}: {update['current']} → {update['recommended']}"
                )
        ...
        if analysis["by_severity"]["high"] > 0:
            report.append(
                "2. ⚡ HIGH: Schedule high-severity updates within 24-48h"
            )
        if analysis["by_severity"]["medium"] > 0:
            report.append(
                "3. 📅 MEDIUM: Plan medium-severity updates in next sprint"
            )
        ...
        report.append("5. 📋 Review and update dependency management policies")
...
"""
    ...
    def run_audit(
        self, ci_mode: bool = False, suggest_fixes: bool = False
    ) -> int:
        ...
        if self.output_format != "json":
            ...
            if analysis["by_severity"]["critical"] > 0:
                print(
                    f"🚨 CRITICAL: {analysis['by_severity']['critical']} packages need immediate attention!"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Security audit for Python dependencies"
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Suggest fixes for vulnerabilities"
    )
    parser.add_argument(
        "--ci",
        action="store_true",
        help="CI mode - exit with error code if vulnerabilities found",
    )
