...
class AutomatedSecurityPipeline:
    ...
    def __init__(self, 
        ...
        self.updater = DependencyUpdater(dry_run=False, backup=True, run_tests=True)
    ...
    def _determine_vulnerability_severity(self, vuln: Dict) -> str:
        ...
        high_keywords = [
            "cross-site scripting", "xss", "csrf", "session hijacking",
            "denial of service", "dos", "buffer overflow", "memory corruption",
            "information disclosure", "data exposure"
        ]
        
        # Medium severity indicators
        medium_keywords = [
            "redos", "regular expression denial", "resource exhaustion",
            "timeout", "redirect", "validation bypass"
        ]
    ...
    def should_auto_update(self, findings: Dict) -> Tuple[bool, str]:
        ...
        if critical_count > 0:
            return True, f"Found {critical_count} critical vulnerabilities requiring immediate update"
        ...
        if high_count > 0 and self.auto_update_rules["high"]:
            ...
            if self.severity_levels["high"] >= severity_threshold:
                return True, f"Found {high_count} high-severity vulnerabilities approved for auto-update"
        
        # Require manual review for medium/low
        if medium_count > 0:
            return False, f"Found {medium_count} medium-severity vulnerabilities requiring manual review"
        ...
        try:
            ...
            if self.updater.write_updated_requirements(updated_lines):
                ...
                if self.updater.install_updates():
                    ...
                    update_results["packages_updated"] = list(self.updater.security_updates.keys())
                ...
                update_results["error"] = "Failed to write updated requirements"
    ...
    def generate_security_report(self, 
        ...
        if "error" not in findings:
            ...
            report.append(f"📦 Total Vulnerable Packages: {findings.get('vulnerable_packages', 0)}")
            report.append("")
            ...
            if findings.get("critical"):
                ...
                for vuln in findings["critical"]:
                    ...
                    if vuln['fix_versions']:
                        report.append(f"  🔧 Fix: {', '.join(vuln['fix_versions'])}")
        ...
        if update_results["attempted"]:
            ...
            if update_results["successful"]:
                ...
                report.append(f"📦 Packages updated: {len(update_results['packages_updated'])}")
        ...
        if findings.get("critical") or findings.get("high"):
            if not update_results["successful"]:
                report.append("1. 🚨 URGENT: Manually review and update critical/high vulnerabilities")
            report.append("2. 🧪 Run full test suite after any manual updates")
            report.append("3. 🚀 Deploy updates to staging environment for validation")
        
        if findings.get("medium") or findings.get("low"):
            report.append("4. 📋 Schedule manual review for medium/low severity vulnerabilities")
        ...
        report.append(f"⏰ Next automated scan: {next_scan.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def run_pipeline(self) -> int:
        ...
        try:
            ...
            # Determine exit code
            if findings.get("critical") and not update_results.get("successful"):
                print("🚨 Exiting with error code due to unresolved critical vulnerabilities")
                return 2  # Critical vulnerabilities not resolved
            elif findings.get("high") and not update_results.get("successful"):
                print("⚠️ Exiting with warning due to unresolved high-severity vulnerabilities")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Automated security update pipeline")
    parser.add_argument("--mode", choices=["scan", "update", "full"], default="full",
                       help="Operation mode (default: full)")
    parser.add_argument("--severity", choices=["critical", "high", "medium", "low"], 
                       default="high", help="Minimum severity to process")
