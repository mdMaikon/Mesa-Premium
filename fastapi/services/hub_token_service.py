...
class HubTokenService:
    ...
    async def extract_token(self, user_login: str, password: str, mfa_code: Optional[str] = None) -> TokenExtractionResult:
        ...
        """
        try:
            logger.info(f"Starting token extraction for user: {mask_username(user_login)}")
            
            # Delegate to refactored service
            result = await self._service.extract_token(user_login, password, mfa_code)
            
            if result.success:
                logger.info(f"Token extraction completed successfully for user: {mask_username(user_login)}")
            else:
                logger.error(f"Token extraction failed for user: {mask_username(user_login)}, Error: {result.message}")
    ...
    def get_token_status(self, user_login: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"Checking token status for user: {mask_username(user_login)}")
            
            # Delegate to repository
            status = TokenRepository.get_token_status(user_login)
            
            if status:
                logger.info(f"Token status retrieved for user: {mask_username(user_login)}, Valid: {status.get('is_valid', False)}")
            else:
                logger.warning(f"No token status found for user: {mask_username(user_login)}")
                
            return status
            
        except Exception as e:
            logger.error(f"Error getting token status for user {mask_username(user_login)}: {e}")
            return None
    
    def get_token_history(self, user_login: str, limit: int = 10) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        """
        try:
            logger.info(f"Getting token history for user: {mask_username(user_login)}")
            ...
            logger.error(f"Error getting token history for user {mask_username(user_login)}: {e}")
            return None
