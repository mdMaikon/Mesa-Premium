...
class MockWebDriver:
    ...
    def _setup_mock_elements(self):
        ...
        # Login form elements
        self._elements["username"] = MockWebElement("input", attributes={"name": "account"})
        self._elements["password"] = MockWebElement("input", attributes={"name": "password"})
        self._elements["login_button"] = MockWebElement("button")
        
        # MFA elements
        self._elements["mfa_fields"] = [
            MockWebElement("input", attributes={"class": "G7DrImLjomaOopqdA6D6dA=="})
            for _ in range(6)
        ]
...
class MockChrome:
    ...
    def __init__(self, options=None, service=None, simulate_success=True, simulate_mfa=True):
...
class HubXPMockScenarios:
    ...
    def partial_mfa_entry():
        ...
        mfa_fields = []
        for i in range(6):
            element = MockWebElement("input", attributes={"class": "G7DrImLjomaOopqdA6D6dA=="})
