...
class TokenResponseFactory(factory.Factory):
    ...
    user_login = factory.LazyAttribute(
        lambda obj: f"USER{obj.id}.A{12345 + obj.id}"
    )
    token = factory.LazyFunction(
        lambda: f"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test_token_{factory.Faker('uuid4')}"
    )
...
class HubCredentialsFactory(factory.Factory):
    ...
    mfa_code = factory.LazyFunction(
        lambda: f"{factory.Faker('random_int', min=100000, max=999999).generate()}"
    )
...
class FixedIncomeDataFactory(factory.Factory):
    ...
    instrumento = factory.Faker(
        "random_element", elements=("CDB", "CRI", "NTN-F", "LCA", "LCI")
    )
    duration = factory.Faker(
        "pyfloat", positive=True, max_value=10, right_digits=2
    )
    indexador = factory.Faker(
        "random_element", elements=("CDI", "IPCA", "SELIC")
    )
    juros = factory.Faker("random_element", elements=("Mensal", "Semestral"))
    primeira_data_juros = factory.Faker(
        "date_between", start_date="today", end_date="+1y"
    )
    isento = factory.Faker("random_element", elements=("Sim", "Não"))
    rating = factory.Faker(
        "random_element", elements=("AAA", "AA", "A", "BBB", "BB")
    )
    vencimento = factory.Faker(
        "date_between", start_date="+1y", end_date="+10y"
    )
    tax_min = factory.LazyAttribute(
        lambda obj: f"{factory.Faker('pyfloat', positive=True, max_value=15, right_digits=2).generate():.2f}%"
    )
    tax_min_clean = factory.Faker(
        "pyfloat", positive=True, max_value=15, right_digits=2
    )
    roa_aprox = factory.Faker(
        "pyfloat", positive=True, max_value=10, right_digits=2
    )
    taxa_emissao = factory.Faker(
        "pyfloat", positive=True, max_value=5, right_digits=2
    )
    publico = factory.Faker(
        "random_element", elements=("Geral", "Qualificado", "Profissional")
    )
    publico_resumido = factory.LazyAttribute(
        lambda obj: {"Geral": "R", "Qualificado": "Q", "Profissional": "P"}[
            obj.publico
        ]
    )
    emissor = factory.Faker("company")
    cupom = factory.LazyAttribute(
        lambda obj: "Jun/Dez" if obj.juros == "Semestral" else ""
    )
    classificar_vencimento = factory.LazyAttribute(
        lambda obj: f"[{((obj.vencimento - datetime.now().date()).days // 365) + 1} Anos]"
    )
    created_at = factory.LazyFunction(datetime.now)
...
class FixedIncomeDatabaseRowFactory(DatabaseRowFactory):
    ...
    ativo = factory.Faker("company")
    instrumento = factory.Faker(
        "random_element", elements=("CDB", "CRI", "NTN-F")
    )
    duration = factory.Faker(
        "pyfloat", positive=True, max_value=10, right_digits=6
    )
    indexador = factory.Faker(
        "random_element", elements=("CDI", "IPCA", "SELIC")
    )
    ...
    tax_min_clean = factory.Faker(
        "pyfloat", positive=True, max_value=15, right_digits=4
    )
    roa_aprox = factory.Faker(
        "pyfloat", positive=True, max_value=10, right_digits=4
    )
    taxa_emissao = factory.Faker(
        "pyfloat", positive=True, max_value=5, right_digits=4
    )
    publico = factory.Faker(
        "random_element", elements=("Geral", "Qualificado", "Profissional")
    )
...
SAMPLE_FIXED_INCOME_STATS = {
    ...
    "earliest_maturity": (datetime.now() + timedelta(days=30))
    .date()
    .isoformat(),
    "latest_maturity": (datetime.now() + timedelta(days=3650))
    .date()
    .isoformat(),
}
