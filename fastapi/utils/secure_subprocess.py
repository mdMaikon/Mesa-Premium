...
class SecureSubprocessRunner:
    ...
    def validate_executable(self, executable: str) -> str:
        ...
        for exe_type, allowed_exes in self.ALLOWED_EXECUTABLES.items():
            if executable in allowed_exes or executable.endswith(
                tuple(allowed_exes)
            ):
        ...
        if executable not in [sys.executable, "python", "python3"]:
            try:
                ...
                if result.returncode not in [
                    ...
                    raise SecureSubprocessError(
                        f"Executable validation failed: {executable}"
                    )
            except (FileNotFoundError, subprocess.TimeoutExpired) as err:
                raise SecureSubprocessError(
                    f"Executable not found or not responding: {executable}"
                ) from err
        ...
        for arg in arguments:
            ...
            if arg.startswith(("./", "/", "../")) or "." in arg and "/" in arg:
                ...
                try:
                    ...
                    if not str(canonical_path).startswith(
                        str(working_canonical)
                    ):
                        raise SecureSubprocessError(
                            f"Path outside working directory: {arg}"
                        )
            ...
            # For non-path arguments, check against whitelist if available
                ...
                if not any(
                    ...
                    logger.warning(
                        f"Argument not in whitelist but allowing: {arg}"
                    )
    ...
    def validate_file_path(self, file_path: str | Path) -> Path:
        ...
        if not str(canonical_path).startswith(str(working_canonical)):
            raise SecureSubprocessError(
                f"File path outside working directory: {file_path}"
            )
    ...
    def run_command(
        ...
        # Validate arguments
        arguments = (
            self.validate_arguments(executable, command[1:])
            if len(command) > 1
            else []
        )
        ...
        try:
            ...
            logger.error(
                f"Command timed out after {timeout}s: {' '.join(safe_command)}"
            )
            raise
        except subprocess.CalledProcessError as e:
            logger.error(
                f"Command failed with exit code {e.returncode}: {' '.join(safe_command)}"
            )
            raise
...
def run_pip_command(
    args: list[str], working_dir: Path | None = None, timeout: int = 600
) -> subprocess.CompletedProcess:
