...
class SensitiveDataSanitizer:
    ...
    def sanitize_message(cls, message: str) -> str:
        ...
        sanitized = cls.SENSITIVE_PATTERNS['password'].sub(r'\1[MASKED]', sanitized)
        ...
        sanitized = cls.SENSITIVE_PATTERNS['credit_card'].sub('****-****-****-****', sanitized)
        sanitized = cls.SENSITIVE_PATTERNS['cpf'].sub('***.***.***-**', sanitized)
...
def mask_sensitive_data(data: Any) -> str:
    ...
    if isinstance(data, str):
        ...
        for key, value in data.items():
            key_lower = key.lower()
            if any(sensitive in key_lower for sensitive in ['password', 'token', 'mfa', 'secret', 'key']):
                masked[key] = '[MASKED]'
            else:
                masked[key] = mask_sensitive_data(value) if isinstance(value, (str, dict, list)) else value
        return str(masked)
