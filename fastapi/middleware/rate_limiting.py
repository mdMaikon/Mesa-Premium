...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if request.url.path == "/api/health" and os.getenv("ENVIRONMENT") == "development":
        response = await call_next(request)
        return response
    ...
    if path == "/api/token/extract":
        rate_limit_str = os.getenv("TOKEN_RATE_LIMIT", RateLimitConfig.TOKEN_EXTRACTION_LIMIT)
    elif path == "/api/fixed-income/process":
        rate_limit_str = os.getenv("FIXED_INCOME_RATE_LIMIT", RateLimitConfig.FIXED_INCOME_LIMIT)
    elif path.startswith("/api/health"):
        rate_limit_str = os.getenv("HEALTH_RATE_LIMIT", RateLimitConfig.HEALTH_CHECK_LIMIT)
    else:
        rate_limit_str = os.getenv("DEFAULT_RATE_LIMIT", RateLimitConfig.DEFAULT_RATE_LIMIT)
    
    # Parse rate limit
    requests_allowed, time_window = RateLimitConfig.parse_rate_limit(rate_limit_str)
    ...
    # Check rate limit
    if not rate_limiter.is_allowed(rate_limit_key, requests_allowed, time_window):
        # Rate limit exceeded
        reset_time = rate_limiter.get_reset_time(rate_limit_key, time_window)
        retry_after = int(reset_time - time.time()) if reset_time else time_window
...
def rate_limit(limit_string: str):
    ...
    def decorator(func):
        ...
        async def wrapper(request: Request, *args, **kwargs):
            # Parse rate limit
            requests_allowed, time_window = RateLimitConfig.parse_rate_limit(limit_string)
